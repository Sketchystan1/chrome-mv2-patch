...
- **macOS** — shipping through the same `chrome-mv2.sh` (it dispatches on the target's container magic). Targets **Chrome 151 and 152** on both slices of the universal `Google Chrome Framework` (`macho-x64` + `macho-arm64`), symbol-verified against Google's official dSYMs; each Mac patches only its own CPU's slice, then the app is ad-hoc re-signed so it launches (§4e).
