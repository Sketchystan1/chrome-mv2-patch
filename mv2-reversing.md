...
5. **Re-verify**: `python scripts/derive_milestone.py <binary> --verify` must print `ALL SITES VERIFIED: True` (a milestone fully covers the build). Run `python scripts/run_tests.py`, then patch a **scratch copy**, confirm on-disk bytes, and GUI-test. Partial or ambiguous layouts are declined by default; developer overrides exist for investigation, not normal releases.
   ...
   **Two traps that a clean `--verify` and a fresh-profile launch both miss (see §7):**
   - **A `cmp <mv>,2 ; jg` may select a _reason string_, not a bool.** Before trusting any site, disassemble **both** branches: a real gate reads the manifest-version field (`[member+0x30]`, x86 `+0x164`) and returns a bool / sets a disable-reason enum; a **string-selector** loads string-literal addresses (`lea rip→.rdata` / `adrp+add` / `mov imm32`) and converges into a message builder guarded by a downstream `CHECK`. Flipping a string-selector crash-loops the browser. It matches exactly once, so the `matches>2`-widening rule never flags it.
   - **The runtime GUI test MUST have a real MV2 extension installed** (force-installed via `ExtensionSettings`/`ExtensionInstallForcelist`, per Gate B), not just a fresh profile. The reason-string path only runs when Chrome evaluates an installed MV2 extension, so a wrongly-flipped string-selector is invisible on an empty profile and only surfaces once an extension is present.
...
| Flipping a `cmp <mv>,2 ; jg` that selects a **reason string**, not a bool ("IsExtensionAffected type!=PLATFORM_APP variant", added in the 154 completeness pass; was in 154/155 + `-x86`/`-linux`/`-arm64`/`-mac` and `154-cft`) | Browser **crash-loops on launch** (`CHECK`/`__debugbreak` → `STATUS_BREAKPOINT 0x80000003`), but **only once a real MV2 extension is installed** — a fresh profile launches fine, so scratch-copy startup tests miss it entirely | The site's (truncated) signature `cmp[reg+0x50],2 ; jg ; cmp byte[reg+0x208],0 ; jne ; mov [reg+0x228]` matched a **message-string selector**, not an enforcement gate: past the member load it does `lea`/`adrp`→`.rdata` on two string literals and `cmove`/`csel`s the MV2-deprecation warning text, then a later `CHECK(reason == expected)` fires. Flipping the branch makes Chrome pick the wrong reason string and the CHECK aborts. Confirmed identical on 153 stable and the 154.8025 build the table was derived from, and on x64/x86/arm64/elf. **Fix (2026-09-09): removed the site from all 11 tables; `154-cft` deleted (without the false site it was a byte-dup of `152` and only produced selection ties). 153 then correctly selects the safe 7-gate `152` table.** **Detection rule: after locating a `cmp,2 ; jg`, disassemble _both_ branches — a real gate reads the manifest-version field (`[member+0x30]`, x86 `+0x164`) and returns a bool / sets a disable-reason enum; a string-selector loads string-literal addresses (`lea rip→.rdata` / `adrp+add` / `mov imm32`) and converges. Reject the string-selector — the `matches>2` widening rule won't catch it because it matches exactly once.** |
